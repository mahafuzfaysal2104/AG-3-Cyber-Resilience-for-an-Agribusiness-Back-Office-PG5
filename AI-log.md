...
## 2026-09-20 — Solving VLAN Setup issues for Wazuh _ (Tanvi)
- Tool: ChatGPT (OpenAI)  
- Prompt: When I give the Vlan ID 0 then the ethernet internet is accessible and I can successfully ping 192.168.50.1 , 10.20.10.1, 10.20.20.1, 10.20.40.1 but can not ping my default gateway 10.20.30.1...But If I change the Vlan ID 0 to 30 ...Then The internet access got lost and I can ping only my default gateway 10.20.30.1 but can not ping the other gateways....I need a proper solution for this that I can all the gateways including my 10.20.30.1 default gateway..  
- What it produced: Recommended not to use the Windows Intel adapter's VLAN ID setting to switch between 0 and 30. Suggested Device Manager → Intel Ethernet → Advanced → VLAN ID → 0. Then VLAN handling should be done by the managed switch + pfSense, not by changing the Windows physical NIC between VLANs. For our topology, the switch port connected to your laptop/MON01 needs to be configured consistently with what MON01 is supposed to receive. As our two designs are valid so it suggested not to mix them.  
- What we changed and why: We tried to solve according to AI's suggestions but it doesn't work.
